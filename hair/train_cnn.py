...
class CustomCNN(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(512, 10)  # Assuming 10 disease classes
        self.relu = nn.ReLU()
...
for epoch in range(epochs):
    ...
    if val_loss < best_val_loss:
        ...
        early_stop_count = 0
        torch.save(model.state_dict(), 'custom_cnn.pth')  # Save best model
    else:
        early_stop_count += 1
...
# Testing the model
model.load_state_dict(torch.load('custom_cnn.pth'))  # Load the best model
model.eval()
test_correct = 0
test_total = 0
